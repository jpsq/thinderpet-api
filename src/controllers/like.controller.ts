...
export const likePet = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (existingLike) {
      ...
      if (!target_pet)
        return badRequest(res, "pet from pet_target_id id not found");
...
export const dislikePet = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (existingLike) {
      ...
      return ok(res, { message: "Dislike successfully." });
    } else {
      return ok(res, { message: "No previous like found." });
    }
  } catch (err) {
    console.error(err);
    return error(res);
